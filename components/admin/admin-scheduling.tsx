...
export function AdminScheduling({ departmentId, isReadOnly = false }: AdminSchedulingProps) {
  ...
  // Real time clock for elapsed time updates
  ...
  useEffect(() => {
    ...
    const fetchTimings = async () => {
      ...
      // Prevent overriding default labels and structure
  ...
  const handleSavePatient = async () => {
    ...
    try {
      const oldP = patients.find(p => p.id === tempData.id);
      const dataToUpdate = { ...tempData };
      
      // Reset tracking time when status changes
      if (oldP && oldP.status !== tempData.status) {
         dataToUpdate.activeMinutes = 0;
         dataToUpdate.statusUpdatedAt = new Date().toISOString();
      }

      await updateDoc(doc(db, 'patients', tempData.id), dataToUpdate); 
      setEditingId(null); 
    } catch (e) {
  ...
  // Calculate live active minutes
  const getLiveActiveMinutes = (patient: any): number => {
    const refDate = patient.statusUpdatedAt || patient.createdAt;
    if (refDate) {
       return Math.floor((now - new Date(refDate).getTime()) / 60000);
    }
    return patient.activeMinutes || 0;
  };
  ...
  // Calculate required minutes for sending
  ...
  // Filter out general category for stages
      ...
      <div className="bg-white border border-[#e8e7f5] rounded-2xl p-6 shadow-sm space-y-5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {Object.keys(timingSettings).map(key => {
            ...
            return (
              <div key={key} className={`border rounded-xl p-4 space-y-3 transition-colors relative ${setting.isActive === 'פעיל' ? 'bg-[#f7f7fc] border-[#e8e7f5]' : 'bg-white border-slate-200'}`}>
                {/* Trash Icon for Custom Timings Centered Top */}
                {isEditingTimings && setting.isCustom && !isReadOnly && (
                  <div className="absolute -top-3 left-1/2 -translate-x-1/2 bg-white rounded-full border border-red-200">
                    <Button variant="ghost" size="sm" onClick={() => handleRemoveCustomTiming(key)} className="h-6 w-6 p-0 text-red-400 hover:text-red-600 hover:bg-red-50 rounded-full">
          ...
          <table className="w-full text-sm text-right min-w-[800px]">
            <thead className="border-b border-[#e8e7f5] text-slate-500 text-xs">
              <tr>
                ...
                <th className="px-4 py-3 font-semibold text-right w-24">זמן בסטטוס</th>
                <th className="px-4 py-3 font-semibold text-center w-64">שליחת הודעה</th>
            ...
            <tbody className="divide-y divide-[#e8e7f5] text-xs">
              {patients.length === 0 ? (
                ...
                const liveActiveMinutes = getLiveActiveMinutes(patient);
                      ...
                      <>
                        ...
                        <td className="px-4 py-2">
                          <Select value={tempData.status || ''} onValueChange={val => setTempData({...tempData, status: val})}>
                            <SelectTrigger className="h-8 text-xs w-full bg-white border-[#e8e7f5]"><SelectValue /></SelectTrigger>
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center justify-center gap-2 w-full">
                            {STAGES.map(stage => {
                              ...
                              // Identify if required time passed
                              ...
                              if (isSent) {
                                return <span key={stage.id} title={stage.label} className="inline-block min-w-[64px] text-center text-[10px] font-bold bg-emerald-100 text-emerald-700 py-1 rounded border border-emerald-200 shrink-0">נשלח</span>;
                              }
                              if (isCurrentStage && !isAutoSent) {
                                return <span key={stage.id} title={stage.label} className="inline-block min-w-[64px] text-center text-[10px] font-bold bg-amber-100 text-amber-700 py-1 rounded border border-amber-200 shrink-0">בתהליך</span>;
                              }
                              return <span key={stage.id} className="inline-block min-w-[64px] text-center text-[10px] text-slate-300 shrink-0">-</span>;
                            })}
                          </div>
                        </td>
                        {!isReadOnly && (
