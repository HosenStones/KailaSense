...
export function AdminHeader({ user, title, gradientClass = "from-[#2a7c7c] to-[#3d9e9e]" }: AdminHeaderProps) {
  ...
  return (
    <header className={`bg-gradient-to-r ${gradientClass} h-14 flex items-center justify-between px-6 sticky top-0 z-50`}>
      ...
      <div className="flex items-center gap-3">
        <div className="flex items-center gap-2 bg-white/15 rounded-lg px-3 py-1.5">
          ...
          <span className="text-white text-sm font-semibold">{user?.fullName || 'טוען...'}</span>
        </div>
