...
export default function HomePage() {
  ...
  return (
    <div className="min-h-screen bg-[#f7f7fc] flex flex-col items-center justify-center p-4 md:p-6" dir="rtl">
      <div className="max-w-md w-full bg-white rounded-3xl shadow-lg border border-[#e8e7f5] p-6 md:p-8 text-center">
        ...
        {step === 1 ? (
          ...
          <div className="animate-in fade-in slide-in-from-right-8 duration-300">
            ...
            <Select onValueChange={setSelectedDept}>
              <SelectTrigger className="w-full h-14 bg-[#f7f7fc] border-[#e8e7f5] text-right rounded-xl text-lg font-medium focus:ring-[#2a7c7c]" dir="rtl">
                <SelectValue placeholder="בחר מחלקה מהרשימה" />
              </SelectTrigger>
              <SelectContent dir="rtl" className="max-h-60">
                {departments.map(dept => <SelectItem key={dept.id} value={dept.id} className="text-right text-base py-3">{dept.name}</SelectItem>)}
              </SelectContent>
            </Select>
            ...
            <Button 
              ...
              className="w-full h-14 mt-6 bg-[#2a7c7c] hover:bg-[#236969] text-white font-bold text-lg rounded-xl transition-all"
            >
              המשך
            </Button>
            
            <button onClick={() => setStep(1)} className="mt-4 text-[#a8a6c4] text-sm hover:text-[#1e1c4a] transition-colors">
              חזרה
            </button>
          </div>
        )}
